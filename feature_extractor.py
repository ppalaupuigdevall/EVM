...
classes_to_names = {'n02096294':'Australian terrier',
...
"""DESCRIPTION: This script creates the feature vectors that will be used to train the Extreme Value Machine and saves 
them in an HDF5 file with the following format:
    Known   - class 0 -- Train
                      -- Test
            - class 1 -- Train
                      -- Test  
...
rootdir = r"C:\Users\user\Ponç\MET\IR\Datasets\Imagenet_Ponc\Uncompressed"
                ...
                for subdir2, dirs2, files2 in os.walk(os.path.join(rootdir, directory)):
                    ...
                    for directory2 in dirs2:
                        ...
                        for subdir3, dirs3, files3 in os.walk(os.path.join(rootdir, directory, directory2)):
                            ...
                            for directory3 in dirs3:

                                full_directory_path = os.path.join(rootdir, directory, directory2, directory3)
                                ...
                                for image_to_extract_fv in list_files4_images:
                                    ...
                                    if(len(np.shape(img))==3):
                                        ...
                                        predicted_class = imagenet_classes[indexx.item()]
                                        ground_truth = classes_to_names[directory2]
                                        #print("Ground Truth = " + ground_truth)
                                        #print("Predicted    = " + predicted_class)
                                        if(ground_truth == predicted_class):
                                            #The prediction is correct
                                            if(len(np.shape(img)) == 3):
                                                # THE IMAGE IS NOT A GRAY IMAGE
                                                feature_vect = get_feature_vector(img, layer, dimension)
                                                feature_vectors_of_that_class.append(feature_vect.numpy())
                                                i += 1
                                                good_predictions+=1

                                    else:
                                        number_of_images -= 1
                                print("Number of "+ ground_truth + " images = " + str(good_predictions))
                                dd = second_group.create_dataset(directory3,(i, dimension[1]), dtype ='f4', data = np.asarray(feature_vectors_of_that_class))
                            break
                    break
            break